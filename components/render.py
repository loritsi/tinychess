...
from components.shared import PIECE_IMAGES
from components.mouse import get_mouse_square
from components.game.captures import get_captured_pieces

def render_board_bg(surface):
    board_surf = pygame.Surface((8,8))
    for x in range(8):
        for y in range(8):
            if (x + y) % 2 == 0:
                board_surf.set_at((x, y), (222,206,237))
            else:
                board_surf.set_at((x, y), (94,91,140))
...
def render_piece(surface, board, square, flip=False):
...
def render_light_at_square(surface, square: chess.square):
    file, rank = chess.square_file(square), chess.square_rank(square)
    rank = 7 - rank
...
def render_mouse_highlight(surface, mouse, board_pos=(0, 0)):
    square = get_mouse_square(mouse, board_pos)
    if square == None:
        return surface
    render_light_at_square(surface, square)
    return surface
...
def get_captured_surfaces(board):
